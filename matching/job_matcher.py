...
def find_role(title: str):
    ...
    for group, data in ROLE_GROUPS.items():
        ...
        for role in data.get("roles", []):
            ...
            # Exact phrase match inside title.
...
def has_experience_mismatch(
    ...
    """Return True only when the posting clearly requires prior experience."""

    explicit = normalize(experience)

    # The structured experience field is the strongest signal.
    if explicit:
        # Accept explicit entry-level ranges beginning at zero.
        if re.search(r"\b0\s*[-–]\s*[1-3]\s*years?\b", explicit):
            return False

        # Accept explicit fresher / entry-level language.
        if contains_pattern(
            explicit,
            [
                r"\bno experience\b",
        ...
        # Reject explicit ranges that start above zero.
        if re.search(r"\b[1-9]\s*[-–]\s*[2-9]\s*years?\b", explicit):
            return True

        # Reject explicit 2+ / 3+ / ... years.
        if re.search(r"\b(?:[2-9]|10)\+?\s*years?\b", explicit):
            return True

        # An explicit numeric experience field that is not one of the
        # accepted entry-level forms should not be treated as fresher.
        if re.search(r"\b[1-9]\s*years?\b", explicit):
            return True

    # For descriptions, reject only wording that clearly describes a
    # required experience threshold. Do not reject unrelated mentions
    # such as a company's history or a tool used for years.
    combined = " ".join(
        [
            normalize(title),
            normalize(description),
    ...
    required_experience_patterns = [
        r"\brequir(?:e|es|ed|ing)\b[^.]{0,80}\b(?:at\s+least\s+)?[1-9]\s*\+?\s*years?\b",
        r"\bminimum\s+(?:of\s+)?[1-9]\s*\+?\s*years?\b",
        r"\b[1-9]\s*\+?\s*years?\s+(?:of\s+)?(?:experience|work experience)\b",
        r"\b[1-9]\s*[-–]\s*[2-9]\s*years?\s+(?:of\s+)?(?:experience|work experience)\b",
        r"\b10\s*\+?\s*years?\s+(?:of\s+)?(?:experience|work experience)\b",
        r"\b[1-9]\s*\+?\s*years?\b[^.]{0,30}\b(?:required|preferred|mandatory|minimum)\b",
    ]

    return contains_pattern(combined, required_experience_patterns)
...
def match_job(
    ...
    experience_text = str(experience_text or "")
    ...
    india_eligible = is_india_location(location)
    ...
    if has_experience_mismatch(
        ...
        return {
            ...
            "india_eligible": True,
            "fresher_friendly": False,
            "reason": "EXPERIENCE_REQUIREMENT_NOT_FRESHER",
        }
...
if __name__ == "__main__":
    ...
    tests = [
        # GOOD MATCHES
        ("SOC Analyst L1", "Hyderabad, India", "0-1 years"),
        ("Junior Cybersecurity Analyst", "Bengaluru, India", "0-2 years"),
        ("Security Analyst", "Pune, India", "Fresher"),
        ("GRC Analyst", "Hyderabad, India", "0-2 years"),
        ("Junior Software Engineer", "Hyderabad, India", "0-2 years"),
        ("Python Developer", "Bengaluru, India", "Fresher"),
        ("API Testing Engineer", "Hyderabad, India", "0-2 years"),
        ("Junior QA Engineer", "Pune, India", "0-1 years"),
        ("Junior DevOps Engineer", "Bengaluru, India", "0-2 years"),
        ("Cloud Support Engineer", "India Remote", "Entry Level"),
        ("Junior Network Engineer", "Hyderabad, India", "0-1 years"),
        ("IT Support Engineer", "Hyderabad, India", "0-2 years"),
        ("Service Desk Analyst", "Bengaluru, India", "Fresher"),
        ("Junior SQL Developer", "Pune, India", "0-2 years"),
        ("Junior Data Analyst", "Hyderabad, India", "0-2 years"),
        ("Junior Machine Learning Engineer", "Bengaluru, India", "0-2 years"),
        ("Cybersecurity Intern", "Hyderabad, India", "Internship"),

        # HARD REJECTS
        ("Senior Software Engineer", "Hyderabad, India", "5+ years"),
        ("Software Engineer III", "Hyderabad, India", "3+ years"),
        ("Software Developer II", "Pune, India", "2-4 years"),
        ("Security Architect", "Bengaluru, India", "7+ years"),
        ("Cybersecurity Manager", "India", "5+ years"),
        ("Expert Network Engineer", "Pune, India", "8+ years"),
        ("QA Engineer", "Pune, India", "1-2 years"),
        ("Technical Recruiter", "Bengaluru, India", "0-2 years"),
        ("Sales Development Representative", "Hyderabad, India", "0-2 years"),
        ("Software Engineer", "United States", "0-2 years"),
        ("Software Engineer", "Remote - Poland", "0-2 years"),
        ("Software Engineer", "Global", "0-2 years"),
        ("Software Engineer", "Worldwide", "0-2 years"),
    
    ]

    print("=" * 75)
    print("TECHNICAL FRESHER MATCHER - HARD FILTER TEST")
    print("=" * 75)
    ...
    for title, location, experience in tests:
        ...
        actual_match = result.get("matched", False)
    ...
    if failed == 0:
        ...
        print("SOME MATCHER TESTS FAILED")
